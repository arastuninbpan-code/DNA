...
// Обратное направление к nearestColorName — название цвета -> конкретный hex, для СОЗДАНИЯ
// раздела голосом/текстом ("создай зелёный раздел школа"): модель не может сама решить, какой
// именно hex подразумевает "зелёный", нужен явный словарь в промпте (см. COLOR_NAME_TO_HEX в
// provider.js, использует эту же таблицу). Где эталонный оттенок NAMED_COLOR_SWATCHES близок к
// одному из PL_COLORS (зелёный/синий/фиолетовый/розовый/жёлтый) — берём ИМЕННО цвет палитры, чтобы
// раздел, созданный голосом, не отличался от созданного вручную; для цветов без близкого аналога
// в PL_COLORS (весь пастельный, ни одного настоящего красного/чёрного/белого) — берём эталон
// напрямую. Соответствие проверено вручную так, чтобы nearestColorName(COLOR_NAME_TO_HEX[x])===x
// (обратный вызов после создания снова узнаёт тот же цвет).
...
export function colorNameToHex(name) {
  return COLOR_NAME_TO_HEX[String(name || '').trim().toLowerCase()] || null;
}
